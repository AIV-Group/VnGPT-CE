    ...
    if conv_history_tokens+max_response_tokens >= token_limit:
        ...
        try:
            ...
            for chunk in chat_generator:
                ...
                yield formatted_history, history_messages, gr.update(value=f"""<i style="color:#3ADF00"><center>Số token của câu hỏi: {num_tokens_from_messages(history_messages)+max_response_tokens}</center></i>""", visible=True)
        except:
            formatted_history = create_formatted_history(state)
